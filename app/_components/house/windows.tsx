...
const Windows = React.forwardRef<HTMLDivElement, TWindowsProps>(({ number_of_windows = 1, parentWindowsContainerClassName }, ref) => {
  ...
  return (
    <div ref={ref} className={`w-[100%] ${parentWindowsContainerClassName}`} data-testid="house-windows-container">
      {windowsArray.map((_, index) => (
        <div key={index} className="w-[40px] h-[35px] border border-black bg-white" data-testid="house-windows" />
      ))}
    </div>
  )
})
